...
22. **常驻 daemon 装载外部编排文档(#6,§6)**:`neoba start` 增 `--presets DIR / --registry FILE / --models FILE`,等价 `neoba.config.json` 顶层 `presets/registry/models` 字段(相对路径相对 config 文件所在目录解析;优先级 flag > config > 缺省);装载与 `workflow check` 同源(`loadPresetsFromDirs` / `loadCapabilityRegistryFile` / `loadModelsFile`,不另写解析防口径分叉),失败(文件不存在 / preset schema 不合 / registry / models 校验不过 / config 字段类型不对)→ 类型化 `StartArtifactLoadError` 退出非 0 不静默降级;缺省(两处均未声明)= 内置 minimal 预设 + 内置注册表、models 走 `<stateDir>/modelscore.json`,行为零漂移。外部编排文档(ae-ops 式四链)自此可常驻交接:CLI 拉起的 daemon 直接受理引用外部 preset 的 `workflow.run`。
